    ...
    <PageShell width="4xl">
      ...
      <div className="grid gap-8 md:grid-cols-[minmax(0,1fr)_320px] md:items-start">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-5 font-mono text-[11px] uppercase tracking-wider text-ink-soft/40">{product.brand}</p>
          ...
          <p className="mt-2 flex items-baseline gap-2">
            ...
            <span className="font-mono text-sm text-ink-soft/40">정가 {product.priceKrw.toLocaleString()}원</span>
          </p>

          <dl className="mt-6 border-t border-ink/10 text-sm">
            {[
              ['발급 방식', providerLabel(product.provider)],
              ['교환 비율', POINT_TO_KRW_NOTE],
              ['판매 상태', product.active ? '교환 가능' : '판매 중지'],
            ].map(([label, value]) => (
              <div key={label} className="flex items-center justify-between border-b border-ink/[0.07] py-3">
                <dt className="text-ink-soft/55">{label}</dt>
                <dd className="font-medium text-ink">{value}</dd>
              </div>
            ))}
          </dl>

          <div className="mt-6 rounded-xl bg-paper/70 p-4 text-[13px] leading-relaxed text-ink-soft/60">
            <p className="mb-1.5 font-semibold text-ink-soft/80">신청 전 확인해 주세요</p>
        ...
        <aside className="md:sticky md:top-24">
          <div className="rounded-xl border border-ink/10 bg-white p-5">
            ...
            {!session ? (
              <>
                <p className="mt-2 text-sm text-ink-soft/55">포인트 교환은 로그인 후 이용할 수 있습니다.</p>
                <Link
                  href="/login"
          ...
          <Link
            href="/shop/orders"
            className="mt-3 block text-center text-sm font-medium text-ink-soft/55 transition-colors hover:text-signal"
          >
            내 교환 내역 보기 →
          </Link>
        </aside>
