...
export default function LessonPlayer({
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-white text-ink-soft">
      ...
      <header className="sticky top-0 z-10 border-b border-hairline bg-white/90 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-3xl items-center gap-4 px-5">
          <Link
            ...
            aria-label="레슨 닫기"
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-fg-muted transition hover:bg-ink/5 hover:text-ink"
          >
      ...
      <footer className="sticky bottom-0 border-t border-hairline bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-3xl items-center gap-4 px-5 py-4">
          {step > 0 ? (
            <button
              type="button"
              onClick={() => setStep((s) => s - 1)}
              className="rounded-full border border-hairline px-5 py-3 text-sm font-semibold text-fg-secondary transition hover:border-ink/25 hover:text-ink"
            >
              ← 이전
            </button>
          ) : (
