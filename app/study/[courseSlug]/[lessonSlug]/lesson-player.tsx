...
const PROSE =
  'text-[15px] leading-relaxed [&_h2]:text-xl [&_h2]:font-bold [&_h2]:tracking-tight [&_h2]:text-ink [&_h2]:mb-4 [&_p]:my-2.5 [&_p]:text-ink-soft/80 [&_li]:text-ink-soft/80 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_code]:font-mono [&_code]:text-[13px] [&_code]:bg-ink/5 [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:rounded [&_pre]:bg-ink [&_pre]:text-white [&_pre]:rounded-xl [&_pre]:p-4 [&_pre]:my-4 [&_pre]:overflow-x-auto [&_pre_code]:bg-transparent [&_pre_code]:p-0 [&_table]:w-full [&_table]:my-3 [&_th]:text-left [&_th]:font-mono [&_th]:text-xs [&_th]:text-ink-soft/50 [&_th]:border-b [&_th]:border-ink/10 [&_th]:px-3 [&_th]:py-2 [&_td]:px-3 [&_td]:py-2 [&_td]:border-b [&_td]:border-ink/5';
...
export default function LessonPlayer({
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-white text-ink-soft">
      ...
      <header className="sticky top-0 z-10 border-b border-ink/[0.06] bg-white/90 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-3xl items-center gap-4 px-5">
          <Link
            href={`/study/${courseSlug}`}
            aria-label="레슨 닫기"
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-ink-soft/50 transition hover:bg-ink/5 hover:text-ink"
          >
          ...
          <div className="h-2 flex-1 overflow-hidden rounded-full bg-ink/[0.07]" role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100}>
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-5 py-10">
        <p className="mb-6 text-xs font-bold uppercase tracking-wider text-brand-600">
          {courseTitle} <span className="text-ink-soft/30">/</span> {lessonTitle}
        </p>
        <div key={step} className="mx-auto w-full max-w-xl" style={{ animation: 'lesson-step-in .28s ease' }}>
          {step === 0 && (
            <h1 className="mb-6 font-display text-3xl font-bold tracking-tight text-ink">{lessonTitle}</h1>
          )}
          <div className={PROSE}>
      ...
      <footer className="sticky bottom-0 border-t border-ink/[0.06] bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-3xl items-center gap-4 px-5 py-4">
          {step > 0 ? (
            <button
              type="button"
              onClick={() => setStep((s) => s - 1)}
              className="rounded-full border border-ink/10 px-5 py-3 text-sm font-semibold text-ink-soft/70 transition hover:border-ink/25 hover:text-ink"
            >
              ← 이전
            </button>
          ) : (
          ...
          <span className="ml-auto font-mono text-xs text-ink-soft/40">
            {step + 1} / {steps.length}
          </span>
          {!isLast ? (
            <button
        ...
        {!loggedIn && isLast && (
          <p className="pb-3 text-center text-xs text-ink-soft/40">
