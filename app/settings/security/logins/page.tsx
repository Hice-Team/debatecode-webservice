  ...
  return (
    <div className="pt-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/settings" className="text-sm text-fg-secondary">설정으로 돌아가기</Link>
      </div>
      ...
      <div className="rounded-xl border border-hairline overflow-hidden">
        {items.length === 0 ? (
          <p className="p-6 text-center text-sm text-fg-muted">검색 결과가 없습니다.</p>
        ) : (
          items.map((e) => (
            <div key={e.id} className="flex items-center gap-3 px-4 py-3 border-b last:border-b-0">
              ...
              <div className="ml-auto font-mono text-[11px] text-fg-muted">{new Date(e.createdAt).toLocaleString('ko-KR', { dateStyle: 'medium', timeStyle: 'short' })}</div>
            </div>
          ))
        )}
      </div>
