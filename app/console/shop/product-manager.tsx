...
export default function ProductManager({ products }: { products: ManagedProduct[] }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {products.length === 0 ? (
        <p className="rounded-xl border border-dashed border-ink/15 px-4 py-14 text-center text-sm text-ink-soft/45">
          등록된 상품이 없습니다. 새 상품을 추가해 보세요.
        </p>
      ) : (
        <ul className="overflow-hidden rounded-xl border border-ink/10 bg-white">
          {products.map((product) => (
            <li key={product.id} className="flex flex-wrap items-center gap-3 border-b border-ink/[0.07] px-4 py-3 last:border-b-0">
              <div className="h-12 w-16 shrink-0 overflow-hidden rounded-lg bg-paper">
                {product.imageUrl ? (
                  ...
                  <span className="grid h-full w-full place-items-center font-mono text-[10px] text-ink-soft/30">
                    no image
                  </span>
                )}
              </div>
              ...
              <span
                className={`shrink-0 rounded-full border px-2 py-0.5 text-[11px] font-semibold ${
                  product.active ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-ink/15 bg-paper text-ink-soft/50'
                }`}
              >
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                <button
                  ...
                  className="rounded-lg border border-ink/15 px-3 py-1.5 text-xs font-medium text-ink-soft/75 transition-colors hover:border-ink/40"
                >
                  수정
                </button>
                <form action={toggleShopProduct}>
                  <input type="hidden" name="id" value={product.id} />
                  <button className="rounded-lg border border-ink/15 px-3 py-1.5 text-xs font-medium text-ink-soft/75 transition-colors hover:border-ink/40">
                ...
                >
                  <input type="hidden" name="id" value={product.id} />
                  <button className="rounded-lg border border-ink/15 px-3 py-1.5 text-xs font-medium text-ink-soft/60 transition-colors hover:border-rose-300 hover:text-rose-600">
                    삭제
                  </button>
                </form>
              </div>
