    ...
    <form action={formAction} className="space-y-3">
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="sm:col-span-2">
          <label htmlFor="draft-title" className="block font-mono text-xs text-ink-soft/60 tracking-wider mb-1.5">제목</label>
          <input id="draft-title" name="title" required minLength={2} placeholder="예: 두 수의 합" className={FIELD} />
        </div>
        <div>
          <label htmlFor="draft-difficulty" className="block font-mono text-xs text-ink-soft/60 tracking-wider mb-1.5">난이도</label>
      ...
      <div>
        <label htmlFor="draft-category" className="block font-mono text-xs text-ink-soft/60 tracking-wider mb-1.5">카테고리</label>
      ...
      <div>
        <label htmlFor="draft-desc" className="block font-mono text-xs text-ink-soft/60 tracking-wider mb-1.5">문제 설명 (마크다운)</label>
      ...
      <div className="rounded-xl border border-ink/15 bg-paper/40 p-4">
        ...
        {donate && (
          <div className="mt-3 space-y-2 border-t border-ink/10 pt-3">
            <p className="rounded-lg bg-white border border-ink/10 px-3 py-2 text-[11px] leading-relaxed text-ink-soft/70">
            ...
            <label htmlFor="draft-signer" className="block font-mono text-xs text-ink-soft/60 tracking-wider">서명 (성명)</label>
      ...
      <div className="flex items-center gap-2">
        ...
        <button type="button" onClick={() => setOpen(false)} className="rounded-xl border border-ink/15 px-4 py-2 text-sm text-ink-soft/70 hover:border-ink/40">취소</button>
      </div>
      <p className="text-[11px] text-ink-soft/50">테스트케이스·스타터코드는 검토 과정에서 검토자와 함께 보완합니다.</p>
    </form>
  );
}
