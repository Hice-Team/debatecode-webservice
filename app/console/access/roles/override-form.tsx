    ...
    <form action={formAction} className="space-y-4 rounded-2xl border border-ink/10 bg-white p-5">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <fieldset className="min-w-0">
          <legend className="mb-1.5 font-mono text-xs tracking-wider text-ink-soft/60">
          ...
          />
          <div className="max-h-56 overflow-y-auto rounded-xl border border-ink/10 divide-y divide-ink/5">
            {filtered.length === 0 && (
              <p className="px-3 py-6 text-center text-xs text-ink-soft/45">해당하는 계정이 없습니다.</p>
            )}
            {filtered.map((c) => (
              <label
                key={c.id}
              ...
              >
                ...
                <span className="shrink-0 font-mono text-[10px] text-ink-soft/45">{c.role}</span>
              </label>
            ))}
          </div>
          {userIds.length > 0 && (
            <button
              type="button"
              onClick={() => setUserIds([])}
              className="mt-1.5 font-mono text-[11px] text-ink-soft/50 underline underline-offset-2 hover:text-ink"
            >
              선택 해제
            </button>
          )}
        ...
        <fieldset className="min-w-0">
          ...
          <div className="max-h-[15.5rem] overflow-y-auto rounded-xl border border-ink/10">
            {GROUP_ORDER.map((group) => {
              const perms = ALL_PERMISSIONS.filter((p) => PERMISSIONS[p].group === group);
              if (perms.length === 0) return null;
              return (
                <div key={group}>
                  <p className="sticky top-0 bg-paper/90 px-3 py-1 font-mono text-[10px] uppercase tracking-widest text-ink-soft/40 backdrop-blur">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_10rem]">
        <fieldset>
          <legend className="mb-1.5 font-mono text-xs tracking-wider text-ink-soft/60">방향</legend>
            ...
            {(
              ...
              >
                ...
                <span className="min-w-0">
                  ...
                  <span className="mt-0.5 block text-[11px] leading-relaxed text-ink-soft/55">{o.desc}</span>
                </span>
              </label>
            ))}
          </div>
        ...
        <div>
          <label htmlFor="grant-days" className="mb-1.5 block font-mono text-xs tracking-wider text-ink-soft/60">
            기간
          </label>
      ...
      <div>
        <label htmlFor="grant-reason" className="mb-1.5 block font-mono text-xs tracking-wider text-ink-soft/60">
