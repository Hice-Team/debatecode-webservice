  ...
  return (
    <div>
      ...
      <section className="mt-8">
        <h3 className="mb-3 text-lg font-bold text-fg">역할 분포</h3>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {ROLES.map((role) => (
            <Link
              key={role}
              href={`/console/access/directory?role=${role}`}
              className="flex items-center gap-3 rounded-[var(--radius-panel)] border border-hairline bg-surface px-4 py-3.5 transition-colors hover:border-brand-300 hover:bg-brand-50/30"
            >
              <span className={`shrink-0 rounded-full border px-2.5 py-1 font-mono text-[11px] ${ROLE_BADGE[role as Role]}`}>
                {ROLE_LABELS[role as Role]}
              </span>
              <span className="ml-auto font-display text-2xl font-bold text-fg">{countByRole.get(role) ?? 0}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <div className="mt-8 grid gap-4 lg:grid-cols-2">
        ...
        <section className="overflow-hidden rounded-[var(--radius-panel)] border border-hairline bg-surface">
          <div className="flex items-center justify-between border-b border-hairline px-5 py-3">
            <h3 className="text-sm font-bold text-fg">개별 권한 오버라이드</h3>
        ...
        <section className="overflow-hidden rounded-[var(--radius-panel)] border border-hairline bg-surface">
          <div className="flex items-center justify-between border-b border-hairline px-5 py-3">
            <h3 className="text-sm font-bold text-fg">3일 내 만료되는 제재</h3>
      ...
      {/* 최근 권한 변경 */}
      <section className="mt-4 overflow-hidden rounded-[var(--radius-panel)] border border-hairline bg-surface">
        <div className="flex items-center justify-between border-b border-hairline px-5 py-3">
          <h3 className="text-sm font-bold text-fg">최근 역할 변경</h3>
