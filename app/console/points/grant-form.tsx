    ...
    <form action={formAction} className="space-y-4 rounded-2xl border border-ink/10 bg-white p-5">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="min-w-0">
          <label htmlFor="grant-search" className="mb-1.5 block font-mono text-xs tracking-wider text-ink-soft/60">
            대상 계정
          </label>
          <input
            id="grant-search"
          ...
          />
          <div className="max-h-48 divide-y divide-ink/5 overflow-y-auto rounded-xl border border-ink/10">
            {filtered.length === 0 && (
            ...
            {filtered.map((t) => (
              ...
              >
                ...
                <span className="shrink-0 font-mono text-[10px] text-ink-soft/45">{t.role}</span>
                <span className="shrink-0 font-mono text-[11px] text-ink-soft/60">
        ...
        <div className="space-y-3">
          <div>
            <label htmlFor="grant-amount" className="mb-1.5 block font-mono text-xs tracking-wider text-ink-soft/60">
              포인트 (음수면 차감)
            </label>
            <input
              id="grant-amount"
            ...
            <div className="mt-1.5 flex flex-wrap gap-1.5">
              {PRESETS.map((p) => (
                <button
                  ...
                  className={`rounded-lg border border-ink/15 px-2.5 py-1 text-[11px] text-ink-soft/70 hover:border-brand-300 hover:text-brand-700 ${FOCUS}`}
                >
                  {p.label}
                </button>
              ))}
          ...
          <div>
            <label htmlFor="grant-memo" className="mb-1.5 block font-mono text-xs tracking-wider text-ink-soft/60">
              사유 (필수 · 원장에 그대로 남습니다)
            </label>
            <input
              id="grant-memo"
          ...
          {selected && valid && (
            <div className="rounded-xl border border-ink/10 bg-paper/40 px-3 py-2.5 text-xs">
