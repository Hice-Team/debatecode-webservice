...
const STATUS_TONES: Record<string, string> = {
  draft: 'border-hairline bg-paper text-fg-secondary',
  ...
  return (
    <>
      ...
      <section className="mt-8" aria-labelledby="compose-title">
        <h2 id="compose-title" className="mb-4 text-lg font-bold text-fg">
          새 메일 작성
        </h2>
      ...
      <section className="mt-10" aria-labelledby="campaigns-title">
        <h2 id="campaigns-title" className="mb-3 text-lg font-bold text-fg">
          발송 목록
        </h2>
        {campaigns.length === 0 ? (
          <p className="rounded-xl border border-dashed border-hairline px-4 py-12 text-center text-sm text-fg-muted">
            아직 작성한 메일이 없습니다.
          </p>
        ) : (
            ...
            {campaigns.map((campaign) => (
              <li key={campaign.id} className="flex flex-wrap items-center gap-3 border-b border-hairline px-4 py-3 last:border-b-0">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-fg">{campaign.subject}</p>
      ...
      <section className="mt-10" aria-labelledby="contacts-title">
        <h2 id="contacts-title" className="mb-3 text-lg font-bold text-fg">
          최근 수신 동의
        </h2>
        {recent.length === 0 ? (
          <p className="rounded-xl border border-dashed border-hairline px-4 py-12 text-center text-sm text-fg-muted">
