  ...
  if (!open) {
    return (
      <button
        ...
        className={`shrink-0 rounded-xl border border-ink/15 px-3 py-1.5 text-xs font-medium text-ink-soft/75 hover:border-amber-300 hover:text-amber-800 ${FOCUS}`}
      >
        경고
      </button>
    );
  ...
  return (
    <form action={warnDebateMate} className="flex w-full flex-wrap items-center gap-2">
      ...
      <input
        ...
        className={`min-w-0 flex-1 rounded-lg border border-ink/15 bg-white px-3 py-1.5 text-xs placeholder:text-ink-soft/40 ${FOCUS}`}
      />
      ...
      <button type="button" onClick={() => setOpen(false)} className="shrink-0 rounded-xl border border-ink/15 px-3 py-1.5 text-xs text-ink-soft/60">
        취소
      </button>
    </form>
  );
  ...
  if (step === 0) {
    return (
      <button
        type="button"
        onClick={() => setStep(1)}
        className={`shrink-0 rounded-xl border border-ink/15 px-3 py-1.5 text-xs font-medium text-ink-soft/75 hover:border-rose-300 hover:text-rose-700 ${FOCUS}`}
      >
        권한 회수
      </button>
    );
  ...
  return (
    <form action={revokeDebateMate} className="w-full space-y-2 rounded-xl border border-rose-200 bg-rose-50/50 p-3">
      ...
      <input
        ...
        className={`w-full rounded-lg border border-ink/15 bg-white px-3 py-1.5 text-xs placeholder:text-ink-soft/40 ${FOCUS}`}
      />
      {step === 2 && (
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          type="button"
          onClick={() => setStep(0)}
          className="rounded-xl border border-ink/15 px-3 py-1.5 text-xs text-ink-soft/60"
        >
          취소
        </button>
      </div>
