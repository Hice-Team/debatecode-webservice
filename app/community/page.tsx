...
function orderByFor(sort: SortKey) {
  if (sort === 'likes') return [{ likes: { _count: 'desc' as const } }, { createdAt: 'desc' as const }];
  if (sort === 'comments') return [{ comments: { _count: 'desc' as const } }, { createdAt: 'desc' as const }];
  if (sort === 'views') return [{ viewCount: 'desc' as const }, { createdAt: 'desc' as const }];
...
export default async function CommunityPage({ searchParams }: PageProps<'/community'>) {
  ...
  const activeBoard = boardParam === ALL_BOARD ? ALL_BOARD : isBoardKey(boardParam) ? boardParam : ALL_BOARD;
  const activePlatform = typeof platform === 'string' ? platform : undefined;
  const query = typeof q === 'string' ? q.trim() : '';
  const pageNum = Math.max(1, Number(typeof page === 'string' ? page : '1') || 1);
  ...
  const viewer = session
    ? { userId: session.userId, role: (await prisma.user.findUnique({ where: { id: session.userId }, select: { role: true } }))?.role ?? 'user' }
  ...
  const where = {
    ...
    ...(activeBoard === 'sns' && activePlatform ? { snsPlatform: activePlatform } : {}),
    ...(query
      ? {
          OR: [
  ...
  const [posts, totalCount, freshCounts] = await Promise.all([
  ...
  const freshByBoard = new Map(freshCounts.map((row) => [row.board, row._count._all]));
  ...
  function buildHref(overrides: Record<string, string | undefined>) {
    ...
    for (const [key, value] of Object.entries(merged)) {
      if (value && !(key === 'board' && value === ALL_BOARD)) params.set(key, value);
    }
    const qs = params.toString();
    return qs ? `/community?${qs}` : '/community';
  }

  const hrefFor = (p: number) => buildHref({ page: p === 1 ? undefined : String(p) });

  const tabs = [
    { key: ALL_BOARD as string, label: '전체', fresh: freshTotal },
    ...BOARDS.map((b) => ({ key: b.key as string, label: b.label, fresh: freshByBoard.get(b.key) ?? 0 })),
  ];
  const overflowTabs = tabs.slice(VISIBLE_TABS + 1);
  // 접힌 게시판을 보고 있으면 그 탭을 앞으로 끌어와 선택 상태가 사라지지 않게 한다
  const activeInOverflow = overflowTabs.find((t) => t.key === activeBoard);
  const shownTabs = activeInOverflow
    ? [...tabs.slice(0, VISIBLE_TABS), activeInOverflow]
    : tabs.slice(0, VISIBLE_TABS + 1);
  const restTabs = overflowTabs.filter((t) => t.key !== activeInOverflow?.key);

  const listTitle = activeBoard === ALL_BOARD ? '전체 게시글' : boardLabel(activeBoard);
  const writeHref = `/community/write?board=${activeBoard === ALL_BOARD ? 'free' : activeBoard}`;

  return (
    <PageShell width="5xl">
      {/* ---------- 커뮤니티 헤더 ----------
           라벨 → 제목 → CTA. 설명 문구 없이 세로를 짧게 가져간다. */}
      <header className="flex items-end justify-between gap-4">
        <div>
          <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.18em] text-brand-600">COMMUNITY</p>
      ...
      {/* ---------- 명예의 전당 ----------
           게시글 탐색을 가리지 않도록 한 줄짜리 링크로만 둔다. */}
      <Link
        href="/hall-of-fame"
        className="group mt-5 flex items-center gap-2 rounded-lg border border-ink/10 bg-white px-3.5 py-2 text-sm transition-colors hover:border-brand-300 hover:bg-brand-50/40"
      >
        <span aria-hidden>🏆</span>
        <span className="font-medium text-ink-soft/75 group-hover:text-signal">
          <I18nSlot k="hof-inline" fallback="이번 시즌 명예의 전당" />
        </span>
        <span className="font-mono text-[11px] text-ink-soft/35">S{season.index}</span>
        <span
          aria-hidden
          className="ml-auto text-ink-soft/30 transition-transform group-hover:translate-x-0.5 group-hover:text-signal"
        >
          →
        </span>
      </Link>
      ...
      {/* ---------- 카테고리 탭 ---------- */}
      <nav aria-label="게시판" className="mt-6 flex items-center gap-1 border-b border-ink/10">
        {shownTabs.map((tab) => {
          const on = tab.key === activeBoard;
          return (
            <Link
              key={tab.key}
              href={buildHref({ board: tab.key, platform: undefined, page: undefined })}
              aria-current={on ? 'page' : undefined}
              className={`-mb-px flex items-center gap-1.5 whitespace-nowrap border-b-2 px-3 py-2.5 text-sm transition-colors ${
                on ? 'border-signal font-semibold text-signal' : 'border-transparent text-ink-soft/55 hover:text-ink'
              }`}
            >
              {tab.label}
              {/* 새 글이 있을 때만 배지 — 평소에는 숫자를 달지 않는다 */}
              {tab.fresh > 0 && !on && (
                <span className="rounded-full bg-brand-100 px-1.5 text-[10px] font-bold text-brand-700">
                  {tab.fresh > 99 ? '99+' : tab.fresh}
                </span>
              )}
            </Link>
          );
        ...
        {restTabs.length > 0 && (
          <details className="relative -mb-px ml-auto">
            <summary className="cursor-pointer list-none px-3 py-2.5 text-sm text-ink-soft/50 transition-colors marker:content-none hover:text-ink">
              더보기 ▾
            </summary>
            <div className="absolute right-0 top-full z-20 mt-1 min-w-40 overflow-hidden rounded-lg border border-ink/10 bg-white py-1 shadow-lg shadow-ink/5">
              {restTabs.map((tab) => (
                <Link
                  key={tab.key}
                  href={buildHref({ board: tab.key, platform: undefined, page: undefined })}
                  className="flex items-center gap-2 px-3 py-2 text-sm text-ink-soft/70 transition-colors hover:bg-brand-50/60 hover:text-signal"
                >
                  {tab.label}
                  {tab.fresh > 0 && (
                    <span className="ml-auto rounded-full bg-brand-100 px-1.5 text-[10px] font-bold text-brand-700">
                      {tab.fresh > 99 ? '99+' : tab.fresh}
                    </span>
                  )}
                </Link>
      ...
      {/* SNS 게시판에서만 나오는 플랫폼 필터 — 탭 아래 한 줄 */}
      {activeBoard === 'sns' && (
        <div className="mt-3 flex flex-wrap gap-1">
          <Link
            href={buildHref({ board: 'sns', platform: undefined, page: undefined })}
            className={`rounded-md px-2.5 py-1 text-xs transition-colors ${
              !activePlatform ? 'bg-brand-50 font-semibold text-signal' : 'text-ink-soft/50 hover:text-ink'
            }`}
          >
          ...
          {SNS_PLATFORMS.map((p) => (
            <Link
              key={p.key}
              href={buildHref({ board: 'sns', platform: p.key, page: undefined })}
              className={`rounded-md px-2.5 py-1 text-xs transition-colors ${
                activePlatform === p.key ? 'bg-brand-50 font-semibold text-signal' : 'text-ink-soft/50 hover:text-ink'
              }`}
            >
              {p.label}
            </Link>
      ...
      <section className="mt-7">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
          <h2 className="font-display text-lg font-bold tracking-tight text-ink">
            {listTitle}
            <span className="ml-1.5 font-mono text-xs font-normal text-ink-soft/35">{totalCount}</span>
          </h2>

          {/* 검색과 정렬은 한 줄에 — 목록을 거르는 도구는 한 자리에 모은다 */}
          <form method="get" className="relative ml-auto w-full sm:w-64">
            {activeBoard !== ALL_BOARD && <input type="hidden" name="board" value={activeBoard} />}
              ...
              {SORTS.map((s) => (
                <Link
                  ...
                  className={`block px-3 py-2 text-sm transition-colors hover:bg-brand-50/60 ${
                    s.key === activeSort ? 'font-semibold text-signal' : 'text-ink-soft/70'
                  }`}
                >
                  {s.label}
                </Link>
        ...
        {posts.length > 0 ? (
          <ul className="mt-3 border-t border-ink/10">
            {posts.map((post) => (
              <PostRow key={post.id} post={post} showBoard={activeBoard === ALL_BOARD} />
            ))}
          </ul>
        ) : (
          <div className="mt-3 border-t border-ink/10 py-20 text-center">
            <p className="text-sm text-ink-soft/45">
              {query ? (
                <I18nSlot k="no-posts-found" fallback={`"${query}"에 해당하는 글이 없습니다.`} />
              ) : (
                <I18nSlot k="no-posts-yet" fallback="아직 게시글이 없습니다. 첫 글을 작성해 보세요." />
              )}
            </p>
            <Link
              href={writeHref}
        ...
        <Pagination page={pageNum} totalPages={totalPages} totalCount={totalCount} hrefFor={hrefFor} />
      </section>
    </PageShell>
  );
}
 ...
 */
function PostRow({ post, showBoard }: { post: PostRowData; showBoard: boolean }) {
  const notice = post.board === 'notice';
  return (
    <li className={`border-b border-ink/[0.07] ${notice ? 'bg-amber-50/60' : ''}`}>
      <Link
      ...
      >
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {!notice && showBoard && (
              <span className="shrink-0 font-mono text-[11px] text-ink-soft/35">{boardLabel(post.board)}</span>
            )}
            {post.type === 'link' && (
              <span className={`shrink-0 rounded-full border px-1.5 py-0.5 text-[10px] font-medium ${platformColor(post.snsPlatform)}`}>
                {platformLabel(post.snsPlatform)}
              </span>
            )}
            {post.secret && (
              <span className="shrink-0 text-[11px]" title="비밀글" aria-label="비밀글">
                🔒
              </span>
            )}
            ...
            <span className="truncate text-[15px] font-semibold text-ink group-hover:text-signal">{post.title}</span>
            {post._count.comments > 0 && (
              <span className="shrink-0 font-mono text-xs font-semibold text-signal">[{post._count.comments}]</span>
            )}
          </div>

          {/* 작성자 · 시각 */}
          <p className="mt-1 truncate font-mono text-[11px] text-ink-soft/40">
            <span data-no-translate>{displayName(post.author, post.anonymous)}</span>
            {' · '}
            <span title={post.createdAt.toLocaleString('ko-KR')}>{timeLabel(post.createdAt)}</span>
          </p>
        </div>

        {/* 반응 — 좋아요·조회 */}
        <div className="hidden shrink-0 items-center gap-3 font-mono text-[11px] text-ink-soft/40 sm:flex">
          <span className={`inline-flex items-center gap-1 ${post._count.likes > 0 ? 'text-rose-500/70' : ''}`}>
            <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 fill-none stroke-current stroke-[1.8]" aria-hidden>
