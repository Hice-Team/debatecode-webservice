  ...
  return (
    <button
      ...
      className={`inline-grid h-8 w-8 place-items-center rounded-md transition-colors ${
        accent ? 'text-brand-600 hover:bg-brand-50' : 'text-fg-secondary hover:bg-paper hover:text-fg'
      }`}
    >
      {children}
    </button>
  ...
  return (
    <div
      className={`flex flex-wrap items-center gap-0.5 bg-paper/60 px-2 py-1.5 ${
        variant === 'flat' ? 'border-b border-hairline' : 'rounded-t-lg border border-b-0 border-hairline'
      }`}
      role="toolbar"
      aria-label="서식"
    >
      ...
      <select
        ...
        className="h-8 rounded-md bg-transparent px-1.5 text-xs text-fg-secondary hover:bg-paper focus:outline-none"
      >
