      ...
      {results.map((r, i) => {
        ...
        return (
          <div key={r.id}>
            ...
            {r.isHidden ? (
              <p className="pl-4 text-white/25">히든 케이스 — 입력과 출력이 숨겨집니다</p>
            ) : (
              <div className="pl-4">
                {r.stdout && <pre className="whitespace-pre-wrap text-white/75">{r.stdout}</pre>}
                {r.status === 'error' ? (
                  ...
                  <p className="text-emerald-300/85">
                    ↩ return {fmt(r.actual)} <span className="text-white/30">({r.timeMs}ms)</span>
                  </p>
                )}
              </div>
            )}
